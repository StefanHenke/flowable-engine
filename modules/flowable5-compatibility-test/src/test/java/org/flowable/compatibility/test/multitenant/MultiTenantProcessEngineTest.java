...
package org.flowable.compatibility.test.multitenant;
...
/**
 * Copied from {@link org.flowable.engine.test.cfg.multitenant.MultiTenantProcessEngineTest} and adapted for usage
 * with flowable 5 compatibility handler.
 */
public class MultiTenantProcessEngineTest {
    ...
    private void setupProcessEngine(boolean sharedExecutor) {
        ...
        DefaultFlowable5CompatibilityHandlerFactory handlerFactory = new DefaultFlowable5CompatibilityHandlerFactory();
        handlerFactory.setCompatibilityHandlerClassName("org.flowable.compatibility.MultiSchemaFlowable5CompatibilityHandler");
        config.setFlowable5CompatibilityHandlerFactory(handlerFactory);
        ...
        config.setDefaultTenant("flowable");
    ...
    private void startProcessInstances(String userId) {
        ...
        Deployment deployment = processEngine.getRepositoryService().createDeployment().addClasspathResource("oneTaskProcess.bpmn20.xml")
                .addClasspathResource("jobTest.bpmn20.xml")
                .deploymentProperty(DeploymentProperties.DEPLOY_AS_FLOWABLE5_PROCESS_DEFINITION, true).deploy();
